...
module ApplicationHelper
  ...
  def web_presence_icons(person)
    %i[blog linkedin facebook twitter skype].map { |site|
  ...
  def col(title, value = nil, last = false, email = false, &_block)
    ...
    if block_given?
      out << content_tag(:td, class: last_class) {
        yield
      }
    else
